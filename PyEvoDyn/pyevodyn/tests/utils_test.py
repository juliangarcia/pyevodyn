...
class TestUtils(unittest.TestCase):
    ...
    def test_uniform_mutation_kernel(self):
        ...
        for _ in range(0, 10):
            ...
            sum_rows = np.array([np.sum(mutation_kernel[i, :]) for i in range(0, rows)])
            np.testing.assert_allclose(sum_rows, np.ones(columns), rtol=0.001,
                                       err_msg="Rows do not sum up to 1. in " + str(mutation_kernel))

    def test_normalize_vector(self):
        # check for 10 random vector that all sum up to 1 when normalized
        for _ in range(0, 10):
            random_vector = np.random.rand(10)
            np.testing.assert_almost_equal(np.sum(utils.normalize_vector(random_vector)), 1, decimal=10,
                                           err_msg="Normalized vectors should sum up to be one")

    def test_kahan_sum(self):
        for _ in range(0, 10):
            random_vector = np.random.rand(10)
            np.testing.assert_almost_equal(np.sum(random_vector), np.sum(random_vector), decimal=10,
    ...
    def test_simulate_discrite_distribution(self):
        ...
        coin = [0.5, 0.5]
        coin_tossing = np.array([utils.simulate_discrete_distribution(coin) for _ in range(0, 500000)])
    ...
    def test_simulate_discrete_distribution_longer(self):
        ...
        np.testing.assert_allclose(dist, result, rtol=0.001, atol=0.0001, err_msg="Bad simulate discrete dist",
                                   verbose=True)
